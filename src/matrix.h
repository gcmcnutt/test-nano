...
public:
    ...
    public:
        ...
        float_prec & operator[](int16_t _column) {
                ...
                ASSERT2((_column >= 0) && (_column < this->_maxColumn) && (_column < MATRIX_MAXIMUM_SIZE), "Matrix index out-of-bounds (at column evaluation)");
            #else
    ...
    Proxy operator[](int16_t _row) {
        #if (defined(MATRIX_USE_BOUND_CHECKING))
            ASSERT2((_row >= 0) && (_row < this->i16row) && (_row < MATRIX_MAXIMUM_SIZE), "Matrix index out-of-bounds (at row evaluation)");
        #else
            #warning("Matrix bounds checking is disabled... good luck >:3");
        #endif
